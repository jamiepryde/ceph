...
void PrimaryLogPG::do_pg_op(OpRequestRef op)
{
  ...
  for (vector<OSDOp>::iterator p = ops.begin(); p != ops.end(); ++p) {
    ...
    case CEPH_OSD_OP_PG_POOL_MIGRATION_RESERVE:
      {
        hobject_t start_obj;
        uint32_t priority = p->op.pool_mig_reserve.priority; // TODO Jamie - not needed
        ...
        try {
          ...
          // TODO Jamie - need to test this code path
...
void PrimaryLogPG::_on_activate_committed(HBHandle *handle)
{
  ...
  if (pool.info.is_pg_migrating(info.pgid.pgid)) {
    ...
    if (is_primary()) {
      //TODO Jamie - this assert seems to fire sometimes when PGs are changing state, do we need it?
  ...
  ThreadPool::TPHandle &handle, bool *work_started)
{
  ...
  // TODO Jamie - delete any objects in target PG > watermark
  ...
  while (ops < max) {
    ...
    *work_started = true;
    ...
    // TODO Jamie - do we need these soid checks
    if (soid.is_min()) {
...
// TODO Jamie - write this function!
