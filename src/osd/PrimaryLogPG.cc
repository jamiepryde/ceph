...
void PrimaryLogPG::on_change(ObjectStore::Transaction &t)
{
  ...
  // clear pool migration reservations on the target PG because peering might
  // not happen and do it on an interval change
  ...
  // Reset pool_migration_info to ensure _on_activate_committed starts fresh
  // after interval change. Without this, the old scan results would cause
  // assertion failures when compared to the new watermark.
  //pool_migration_info.reset(hobject_t());
}
...
struct C_Migrate : public Context {
  ...
  {}
  void finish(int r) override {
    ldpp_dout(pg, 1) << "C_Migrate::finish r=" << r << " oid=" << oid << " action=" << action << dendl;
    if (r == -ECANCELED) {
      ldpp_dout(pg, 1) << "C_Migrate::finish ECANCELED, cleaning up" << dendl;
      std::scoped_lock l{*pg};
      pg->pool_migrations_in_flight.erase(oid);
      auto i = pg->recovering.find(oid);
      if (i != pg->recovering.end()) {
        pg->recovering.erase(i);
    ...
    if (last_peering_reset != pg->get_last_peering_reset()) {
      ldpp_dout(pg, 1) << "C_Migrate::finish peering reset mismatch, cleaning up" << dendl;
      pg->pool_migrations_in_flight.erase(oid);
      auto i = pg->recovering.find(oid);
      if (i != pg->recovering.end()) {
        pg->recovering.erase(i);
    ...
    if (r < 0) {
      ...
      ldpp_dout(pg, 1) << "C_Migrate::finish error r=" << r << ", calling handle_pool_migration_copy_failure" << dendl;
      ...
      ldpp_dout(pg, 1) << "C_Migrate::finish success, action=COPY_DELETE_NEXT, calling pool_migration_source_start_delete" << dendl;
      ...
      ldpp_dout(pg, 1) << "C_Migrate::finish success, action=COPY_HEAD, calling finish_recovery_op" << dendl;
      pg->finish_recovery_op(oid);
    }
    ldpp_dout(pg, 1) << "C_Migrate::finish complete" << dendl;
  }
};
...
struct C_PoolMigrationReservationCallback : public Context {
  ...
  void finish(int r) override {
    ldpp_dout(pg, 1) << "C_PoolMigrationReservationCallback::finish r=" << r << dendl;
    if (r == -ECANCELED) {
      ldpp_dout(pg, 1) << "C_PoolMigrationReservationCallback::finish ECANCELED, returning" << dendl;
      return;
    }
    std::scoped_lock l(*pg);
    if (last_peering_reset != pg->get_last_peering_reset()) {
    ...
    ldpp_dout(pg, 1) << "C_PoolMigrationReservationCallback::finish calling on_pool_migration_source_reserved" << dendl;
...
void PrimaryLogPG::pool_migration_source_start_delete(hobject_t oid)
{
  ...
  if (pool_migration_watermark == oid) {
    ...
    // watermark object already queued by pool_migration_source_start_delete and deleted during peering reset by apply_and_flush_repops calling the delete's callback function?
  ...
  ctx->register_on_finish(
            [this, oid]() {
              ...
              if (i != recovering.end()) {
                ...
                // Object has been removed from the recovering list,
                // probably due to the copy being cancelled.
...
void PrimaryLogPG::pool_migration_target_delete(const pg_t &source_pg, const hobject_t &watermark)
{
  ...
  while (current < pg_end && !done) {
    ...
    for (const auto& obj : objects) {
      ...
      // To compare objects from different pools, we need to normalize them to the same pool.
