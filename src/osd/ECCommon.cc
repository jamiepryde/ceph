...
void ECCommon::RecoveryBackend::handle_recovery_read_complete(
  ...
  if (sinfo.supports_partial_writes()) {
    aligned_size = ECUtil::align_next(op.obc->obs.oi.size);
  } else {
    aligned_size = ECUtil::align_next(op.obc->obs.oi.size, sinfo.get_chunk_size());
  }
...
void ECCommon::RecoveryBackend::continue_recovery_op(
  ...
  while (1) {
    ...
    case RecoveryOp::IDLE: {
      ...
      if (op.obc) {
        uint64_t read_to_end;
        if (sinfo.supports_partial_writes()) {
          read_to_end = ECUtil::align_next(op.obc->obs.oi.size) -
            op.recovery_progress.data_recovered_to;
        } else {
          read_to_end = ECUtil::align_next(op.obc->obs.oi.size, sinfo.get_chunk_size()) -
            op.recovery_progress.data_recovered_to;
        }
