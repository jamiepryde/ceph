  ...
  projected_size(soi?soi->size:(oi?oi->size:0))
{
  ...
  if (op.truncate && op.truncate->first < orig_size) {
    ...
    uint64_t next_align;
    if (sinfo.supports_partial_writes()) {
      next_align = ECUtil::align_next(op.truncate->first);
    } else {
      next_align = ECUtil::align_next(op.truncate->first, sinfo.get_chunk_size());
    }
...
void ECTransaction::Generate::truncate() {
  ...
  if (entry && !op.is_fresh_object()) {
    ...
    for (auto &&[shard, eset]: truncate_eset) {
      ...
      uint64_t start_align_prev;
      if (sinfo.supports_partial_writes()) {
        start_align_prev = ECUtil::align_prev(start);
      } else {
        start_align_prev = ECUtil::align_prev(start, sinfo.get_chunk_size());
      }
    ...
    for (auto &&[shard, eset]: truncate_eset) {
      ...
      auto &t = transactions.at(shard);
      uint64_t start = eset.range_start();
      uint64_t start_align_next;
      if (sinfo.supports_partial_writes()) {
        start_align_next = ECUtil::align_next(start);
      } else {
        start_align_next = ECUtil::align_next(start, sinfo.get_chunk_size());
      }
