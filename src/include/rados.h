...
#define __CEPH_FORALL_OSD_OPS(f)					    \
	...
	f(PG_POOLMIG_RESERVE_TAKE,    __CEPH_OSD_OP(WR, PG, 8), "pg-poolmig-reserve-take") \
	f(PG_POOLMIG_RESERVE_RELEASE, __CEPH_OSD_OP(WR, PG, 9), "pg-poolmig-reserve-release") \
	...
	union {
		...
		struct {
			__u32 priority;
		        __le64 num_bytes;
		        __le64 num_objects;
		} __attribute__ ((packed)) pool_mig_reserve;
	} __attribute__ ((packed));
	__le32 payload_len;
} __attribute__ ((packed));
